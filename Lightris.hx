...
class Object2d extends h2d.Object implements h2d.domkit.Object {}
...
class Block {
	...
	public var x: Int;
	public var y: Int;
	public var obj: h2d.Bitmap;
	var inf: Data.Mino;

	public function new(x, y, inf: Data.Mino, ?parent) {
		this.inf = inf;
		obj = new h2d.Bitmap(inf.gfx.toTile(), parent);
		obj.dom = domkit.Properties.create("bitmap", obj);
...
class Piece {
	...
	public function rotateCW() {
		...
		for (b in blocks) {
			var dx = b.x - px;
			var dy = b.y - py;
			var nx = Math.round(dy + px);
			var ny = Math.round(-dx + py);
			b.x = nx;
			b.y = ny;
		}
		rotation++;
...
class Lightris extends hxd.App {
	...
	var gridCont : Object2d;
	var gridGraphics : h2d.Graphics;
	var boardObj : Object2d;
	var tf : h2d.Text;
	...
	override function init() {
		...
		boardObj = new Object2d(gridCont);
